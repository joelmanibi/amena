...
function TrainingsPaginationControls({ page, totalPages, query, copy }) {
  if (totalPages <= 1) {
    return null;
  }
  ...
  return (
    <nav className="flex flex-wrap items-center justify-center gap-2" aria-label={copy.paginationLabel}>
      <Link
        href={buildPageHref({ page: Math.max(page - 1, 1), query })}
      ...
      {pages.map((item) => (
        <Link
          key={item}
          href={buildPageHref({ page: item, query })}
          className={cn(
      ...
      <Link
        href={buildPageHref({ page: Math.min(page + 1, totalPages), query })}
...
export { TrainingsPaginationControls };